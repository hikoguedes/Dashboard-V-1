...
caminho_arquivo = r"claro_HG_VENDAS_PY.xlsx"  # Altere o caminho aqui
...
# PÁGINA GRÁFICOS VIABILIDADE
elif pagina == 'GRÁFICOS VIABILIDADE':
    st.title('📊 Gráficos de Viabilidade')
    if not df_filtrado.empty:
        fig, ax = plt.subplots(figsize=(12, 6))
        sns.barplot(data=df_filtrado, x='Origem da venda',
                    y='Valor vendido', ax=ax)
        plt.title('Viabilidade por Origem da Venda')
        st.pyplot(fig)
    else:
        st.write("Nenhum dado encontrado.")
...
elif pagina == 'GRÁFICOS TABELA':
    st.title('📊 Gráficos Tabela')
    if not df_filtrado.empty:
        # Adicione o código para a página GRÁFICOS TABELA aqui
        pass
    else:
        st.write("Nenhum dado encontrado.")
...
# Título do app
st.title("📊 Análise de Vendas por Campanha com Layout Personalizado")
...
total_registros = df_filtrado.shape[0]
percent_a_vista2 = round((quant_a_vista2 / total_registros)
                         * 100) if total_registros != 0 else 0
...
tipos_unidade = df_a_vista['Tipo unidade semanas'].dropna().unique().tolist()
tipos_unidade.insert(0, 'Todos')

tipo_unidade_selecionado = st.selectbox(
    '🏡 **Selecione o Tipo de Unidade (semanas):**',
    tipos_unidade,
    key='filtro_tipo_unidade_a_vista'
)
...
vendas_estado = df_filtrado.groupby('UF')['Valor vendido'].sum().reset_index()
st.dataframe(vendas_estado)

st.subheader("📊 Gráfico de Vendas por Estado")
...
st.subheader("📋 Clientes por Campanha")
clientes_campanha = df_filtrado.groupby('Campanha')['CLIENTE'].nunique(
).reset_index().rename(columns={'CLIENTE': '# Clientes'})
st.dataframe(clientes_campanha)
...
# Subtítulo para gráfico adicional
